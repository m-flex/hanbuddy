...
export default function Learn({ progress, updateProgress }) {
  ...
  const cards = useMemo(() => {
    ...
    const previousItems = getPreviousItems(stage, Number(lessonIdx))
    const reviewCount = Math.min(2, previousItems.length)
    const shuffledPrev = [...previousItems].sort(() => Math.random() - 0.5)
    const reviewCards = shuffledPrev.slice(0, reviewCount)
    return [...currentLesson.items, ...reviewCards]
  }, [stage, lessonIdx])

  const item = cards[currentIndex]
  const isLast = currentIndex === cards.length - 1
  const isReviewCard = currentIndex >= lesson.items.length
        ...
        <div className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full bg-slate-800 border border-slate-700">
          {isReviewCard ? (
            ...
            <span className="text-primary-400">
              New: {currentIndex + 1} of {lesson.items.length}
            </span>
          )}
        </div>
      </div>
          ...
          <div className={`bg-slate-800/80 border rounded-3xl p-8 text-center min-h-[360px] flex flex-col items-center justify-center relative overflow-hidden
            ...
            <AnimatePresence mode="wait">
              {!flipped ? (
                ...
                >
                  <span className="hangul text-5xl sm:text-6xl font-black text-white mb-4">{item.word}</span>
                ...
                >
                  <span className="hangul text-4xl font-bold text-white">{item.word}</span>
                  ...
                  <div className="text-xl font-bold text-primary-400">{item.roman}</div>
                  <div className="text-lg text-success-400 font-medium">{item.meaning}</div>

                  {item.hint && (
                    <div className="text-sm text-amber-300/90 bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-xl text-center">
                      {item.hint}
                    </div>
                  )}

                  {item.example && (
                    <div className="text-sm bg-slate-700/50 px-4 py-3 rounded-xl w-full">
                      <div className="hangul text-white mb-1">{item.example.kr}</div>
                      <div className="text-slate-400 text-xs">{item.example.en}</div>
                    </div>
                  )}
                </motion.div>
              )}
