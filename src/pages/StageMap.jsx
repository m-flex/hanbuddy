...
import { Lock, BookOpen, HelpCircle, ChevronRight, ChevronDown, RotateCcw, CheckCircle, MessageSquare, Calendar, Zap, Target, Ear } from 'lucide-react'
import { STAGES, getLessons } from '../data/korean'
  ...
  const [expandedStage, setExpandedStage] = useState(() => {
    for (const stage of STAGES) {
      if (progress.stagesUnlocked.includes(stage.id)) {
        ...
        const allDone = lessons.every((_, i) => getMilestones(progress, stage.id, i).quizPassed)
    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400">Master Korean step by step</p>
        </div>
        {hasReviewItems && (
          <Link to="/review">
            <motion.button
                        ...
                        <div className="flex-1 h-1.5 bg-slate-700/50 rounded-full overflow-hidden">
                          <motion.div
                            ...
                            className={`h-full rounded-full bg-gradient-to-r
                              ...
                              ${stage.color === 'cyan' ? 'from-cyan-500 to-cyan-400' : ''}
                              ${stage.color === 'amber' ? 'from-amber-500 to-amber-400' : ''}
                              ${stage.color === 'rose' ? 'from-rose-500 to-rose-400' : ''}
                            `}
                          />
                        </div>
                  ...
                  >
                    <div className="px-5 pb-5 space-y-2">
                      {lessons.map((lesson, li) => {
                        ...
                        const allDone = ms.learned && ms.quizPassed
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className={`text-xs hangul ${available ? 'text-slate-400' : 'text-slate-600'} truncate`}>
                                  {lesson.preview}
                                </div>
                              </div>
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <PracticeLink to="/listening" icon={Ear} color="green" title="Listening Drill" desc="Audio comprehension practice" />
          <PracticeLink to="/weak-drill" icon={Target} color="red" title="Weak Areas" desc="Practice your problem words" />
        </div>
      </div>
    </motion.div>
  )
